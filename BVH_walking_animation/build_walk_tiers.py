...
def main() -> None:
    ...
    existing_bvh = sorted(ROOT.rglob("*.bvh"))
    buckets = {
    ...
    for bvh_path in existing_bvh:
        ...
        relative_path = bvh_path.relative_to(ROOT.parent).as_posix()
        buckets[bucket].append((relative_path, description))

    for filename, entries in buckets.items():
        write_list(ROOT / filename, entries)

    write_summary(ROOT / "walk_tiers_summary.md", buckets)
...
def read_descriptions() -> dict[str, str]:
    ...
    for index_path in ROOT.rglob("cmu-mocap-index-text.txt"):
